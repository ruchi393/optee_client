		  ...
		  CK_BYTE_PTR wrapped_key, CK_ULONG_PTR key_len)
{
	...
	if (!mechanism || !key_len ||
	    (key_len && *key_len && !wrapped_key))
		return CKR_ARGUMENTS_BAD;
	...
	if (key_len && *key_len) {
		out_shm = ckteec_register_shm(wrapped_key, *key_len,
					      CKTEEC_SHM_OUT);
	} else {
		/* Query output data size */
		out_shm = ckteec_alloc_shm(0, CKTEEC_SHM_OUT);
	...
	if (key_len && (rv == CKR_OK || rv == CKR_BUFFER_TOO_SMALL))
		*key_len = out_size;
		    ...
		    CK_ULONG count, CK_OBJECT_HANDLE_PTR handle)
{
	...
	struct serializer sattr = { 0 };
	uint32_t session_handle = session;
	uint32_t obj_handle = unwrapping_key;
	size_t ctrl_size = 0;
	uint32_t key_handle = 0;
	char *buf = NULL;
	size_t out_size = 0;
